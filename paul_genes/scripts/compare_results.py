...
def compare_eval_summaries(dir1, dir2, outdir, batch_name, compare_each_batch):
    ...
    # different output dirs based on what's being compared
    if compare_each_batch:
        out_path = os.path.join(outdir, batch_name, "eval_summary.txt")
    elif batch_name == None:
        out_path = os.path.join(outdir, "eval_summary.txt")
    else:
...
def compare_eval_files(indir, dir1, dir2, options, outdir, totals, compare_each_batch):
    ...
    for i in range(len(print_statements)):
        ...
        if compare_each_batch: # if user wants to compare each batch individually
        ...
        for line in check_in[i]: # iterate through each line in the .eval files
            ...
            if line not in check_against[i]: # if the line in the first file is not in the second
                ...
                # output for individual comparisons
                if compare_each_batch:
                    with open(os.path.join(outdir, current_comparison[i]+".txt"), "a") as fout:
                        fout.write("\n\n")
...
def compare_log_phi_marked(dir1, dir2, indir, outdir, totals, compare_each_batch):
    ...
    for filepath in log1_big: # go through each file in the log
    ...
    # output number of different tags for the dirs
    if compare_each_batch:
...
# output the totals
def total_up(outdir, totals, subdirs_exist):

    # set up output directory
    outdir = os.path.join(outdir, "tags/")
    ...
    # output files for each comparison (see current_out)
...
def main():
    help_str = """
        ...
        files. This script will compare both individual and batched runs.
    ...
    ap.add_argument("-o", "--outputdir",
                    default=".",
                    help="""The path to the folder where the output folder will
    ...
    # create individual output dirs if user wants to compare everything individually
    if compare_each_batch:
    ...
    if subdirs_exist: # walk through each subdir
    ...
    else: # if there aren't any subdirs
        ...
        compare_eval_files(indir, dir1, dir2, options, outdir, totals, compare_each_batch)

        if "phi_marked" in options:
            compare_log_phi_marked(dir1, dir2, indir, outdir, totals, compare_each_batch)

    total_up("compare_results_out_"+dir2.split("/")[-2], totals, subdirs_exist)
